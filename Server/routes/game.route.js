...
router.route("/getAll").get((req, res, next) => {
  GameModel.find()
    .populate(
      "whiteTeam.offense whiteTeam.defense blackTeam.offense blackTeam.defense"
    )
    .exec((error, data) => {
      if (error) {
        return next(error);
...
router.route("/create").post(async (req, res, next) => {
  let data = req.body;
  let gameObject = {
    blackScore: parseInt(data.blackScore),
    whiteScore: parseInt(data.whiteScore),
    whiteTeam: {
  ...
  gameObject.rating = ratingDelta;
  GameModel.create(gameObject, async (error, data) => {
    if (error) {
      return next(error);
    } else {
